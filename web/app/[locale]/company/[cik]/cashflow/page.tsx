...
import { getCashFlow, formatValue } from "@/lib/api";
...
const TAG_DISPLAY: Record<string, string> = {
  ...
  PaymentsForRepurchaseOfEquity: "Stock Buybacks",
...
export default async function CashflowPage({ params }: { params: Promise<{ cik: string }> }) {
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-gray-700">
                <th className="text-left text-gray-400 font-medium py-2 pr-4 min-w-[200px]">Item</th>
                {dates.map((d) => (
            ...
            <tbody>
              {ordered.map((series) => {
                ...
                const isCapex = series.tag === "PaymentsToAcquirePropertyPlantAndEquipment";
                return (
                  <tr key={series.tag} className="border-b border-gray-800 hover:bg-gray-800/30">
                    <td className={`py-2 pr-4 text-gray-300 ${isCapex ? "pl-4 text-gray-400" : "font-medium"}`}>
                      {label}
                    </td>
                    {dates.map((d) => {
                      ...
                      return (
                        <td key={d} className={`py-2 px-2 text-right font-mono text-xs ${isNeg ? "text-red-400" : "text-green-400"}`}>
                          {val}
                        </td>
                      );
                    })}
